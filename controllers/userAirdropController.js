...
const { handleAddress } = require("../utils/handleChain");
    ...
    if (userSubcription.length > 0) {
      ...
      try {
        ...
        if (userSubcription.length > 0) {
          ...
          return res
            .status(400)
            .json({ error: "User is not an Active Subscriber" });
        }
      } catch (error) {
        return res.status(500).json({ error: "Error during airdrop process", msg: error });
      }
    }
  } else {
